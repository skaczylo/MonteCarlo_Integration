...
Once we have the rectangle $[a, b] \times [0, M]$, we generate \textit{num\_points} random points inside it and check for each point whether it falls under the area formed by the graph and the $X$-axis.
...
In this notebook, two implementations of this Monte Carlo method are provided: one using traditional programming methods (loops, conditionals, etc.), and another using \textbf{NumPy vectorized functions}. The execution times of both implementations are compared to evaluate the efficiency gains from using NumPy.
